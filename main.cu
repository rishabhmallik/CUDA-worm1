...
__global__ void worm_update(int *k, int *k_, int *a, double mu, double *I_val, int *l, int worms){
...
double phi2(int k[], int a[], double I[]){
    double sum=0;
    int x[d];
    for (int i=0;i<Nt;i++){
        for (int j=0;j<Ns;j++){
...
void update(int *k, int *k_, int *a, int *a_, double mu, double *I_val, int *l, int worms, bool flag2){
    ...
    worm_update<<<blocks,blockSize>>>(k,k_,a,mu,I_val,l,worms);
}
...
float errorjack(double xi[], int configs){
...
int main(int argc, char **argv)
{
    ...
    ofstream data, data1, data3, data4, data5;
    
	string filename1="mu_vs_n_loops="+to_string(worms)+".txt"
		, filename2="mu_vs_phi2_loops="+to_string(worms)+".txt";
	data.open(filename1);
	data1.open(filename2);
          
    for (int g=0; g<mu_n; g++){
    
    blocks=Nt*Ns*d/blockSize+1;
    init_lattice<<<blocks,blockSize>>>(k,0,Nt*Ns*d);
    init_lattice<<<blocks,blockSize>>>(k_,0,Nt*Ns*d);
    init_lattice<<<blocks,blockSize>>>(a,0,Nt*Ns*d);
    init_lattice<<<blocks,blockSize>>>(a_,0,Nt*Ns*d);
    blocks=Nt*Ns/blockSize+1;
    init_lattice<<<blocks,blockSize>>>(l,0,Nt*Ns);
    
        for (int i=0; i<equil; i++){
            update(k,k_,a,a_,mu,I_val,l,worms,false);
        }
        ...
        for (int i=0; i<configs; i++){
            for (int j=0; j<gaps; j++){
                update(k,k_,a,a_,mu,I_val,l,worms,true);
            }
